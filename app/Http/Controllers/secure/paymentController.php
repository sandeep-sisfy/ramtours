...
class paymentController extends Controller
{
    ...
    function connect_to_cardcom($item_name,$tran_id, $amount){
		    $cart= session()->get('rami_pack_cart');
	    	$passengers=session()->get('rami_pack_passengers');
	    	$payee=session()->get('rami_pack_payee');
	    	if((empty($cart))||(empty($passengers))||(empty($payee))){
	...
	function verify_deal($LowProfileCode){
  ...
  function payment_verify(Request $request){
   ...
   function order_pdf_generate($order_id){
  ...
  function gererater_pdf_html_fhc_pack($cart, $order, $type){
   ...
   function gererater_pdf_html_flight_schedule($cart, $order, $type){
