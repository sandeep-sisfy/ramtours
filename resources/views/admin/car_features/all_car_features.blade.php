        ...
        <div class="row clearfix">
            <div class="col-lg-12">
                <div class="card feature_list">
                   <div class="header">
                       <h4>Car Features&nbsp;<span class="badge badge-primary">{{$all_count}}</span></h4>
                       <button type="button" class="btn btn-raised btn-primary waves-effect rami_admin_add_new_btn"><a href="{{url('admin/car-features/create')}}">Add-Features</a></button>
                    </div>
                    <div class="body table-responsive">
                        <table class="table table-bordered table-striped table-hover dataTable pack_table">
                            ...
                            <tbody>                              
                                ...
                                <tr>
                                    <td>{{++$loop->index}}</td>
                                    <td>{{$feature->car_feature}}</td>
                                    <td>
                                    <div class="btn-group btn-group-xs" role="group" aria-label="Extra-small button group">
                                        <a href="{{ url('admin/car-features/'.$feature->id.'/edit') }}" class="btn btn-default waves-effect waves-float waves-green"><i class="zmdi zmdi-edit" title="Edit"></i></a>
                                    </div>
                                    <div class="btn-group btn-group-xs" role="group" aria-label="Extra-small button group">
                ...
                <form id='del_feature_form' method="POST" action="{{url('admin/car-features')}}" style="display: none">
                    {{ csrf_field() }}
                    {{method_field('DELETE')}}
                </form>
            </div>
...
@parent
    ...
    <script type="text/javascript">
        $('.feature_list').on('click', '.feature_del_btn', function(event) {
            ...
            if(confirm('Are you sure to delete this Car Feature.')){   
