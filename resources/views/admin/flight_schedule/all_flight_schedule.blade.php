        ...
        <div class="col-lg-12">
            <div class="card flight_sche_list">
                <div class="header">
                    ...
                    <button type="button" class="btn btn-raised btn-primary waves-effect rami_admin_add_new_btn"><a
                            href="{{url('admin/flight-schedule/create')}}">Add-Schedule</a></button>
                    @if(!empty($trash_count))
                    <button type="button"
                        class="btn btn-raised btn-primary waves-effect rami_admin_left_btn rami_admin_btn"><a
                            href="{{url('admin/flight-schedule/trash')}}">view Trash &nbsp;<span
                                class="badge badge-danger">{{$trash_count}}</span></a></button>
                    @endif
                </div>
                    ...
                    <table class="table table-bordered table-striped table-hover dataTable pack_table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><span class="text-muted">$
                                        {{ get_rami_flight_price_for_single_flight($flight_schedule->id)}}</span></td>

                                <td><span class="text-muted">{{$flight_schedule->flight_profit}}</span></td>
                                <td><span class="text-muted">{{$flight_schedule->package_profit}}</span></td>

                                @php
                                if($flight_schedule->flight_sche_status==1){
                                $flight_sche_status='Active';
                                }else{
                                $flight_sche_status='Inactive';
                                }
                                @endphp
                                <td>
                                    <div class="btn-group btn-group-xs" role="group"
                                        aria-label="Extra-small button group">
                                        <a target="_blank" href="{{ url('flight-detail/'.$flight_schedule->id) }}"
                                    ...
                                    <div class="btn-group btn-group-xs" role="group"
                                        aria-label="Extra-small button group">
                                        <a href="{{ url('admin/flight-schedule/'.$flight_schedule->id.'/edit') }}"
                                            class="btn btn-default waves-effect waves-float waves-green">
                                    ...
                                    <div class="btn-group btn-group-xs" role="group"
                                        ...
                                        <a href="javascript:void(0);"
                                            class="btn btn-default waves-effect waves-float waves-red flight_sche_del_btn"
                                            item_id="{{$flight_schedule->id}}">
            ...
            <form id='del_flight_sche_form' method="POST" action="{{url('admin/flight-schedule')}}"
                style="display: none">
                {{ csrf_field() }}
                {{method_field('DELETE')}}
            </form>
