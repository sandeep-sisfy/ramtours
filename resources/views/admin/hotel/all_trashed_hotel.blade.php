        ...
        <div class="row clearfix">
            <div class="col-lg-12">
                <div class="card hotel_list">
                    <div class="header">
                       <h4>Private Hotels&nbsp;<span class="badge badge-primary">{{$all_count}}</span></h4>
                       @if(!empty($all_count))
                    ...
                    <div class="body table-responsive">
                        <table class="table table-bordered table-striped table-hover dataTable pack_table">
                            ...
                            <tbody>                              
                                @foreach($trash_hotel as $hotel)
                                <tr>
                                    <td>{{++$loop->index}}</td>
                                    <td>{{$hotel->hotel_name}}</td>
                                    <td>{{$hotel->hotel_code}}</td>
                                    <td>
                                       @if(!empty($hotel->hotel_type_name))
                                       {{$hotel->hotel_type_name->hotel_type}}
                                       @endif
                                    </td>
                                    <td>{{$hotel->hotel_contact}}1</td>
                                    <td>
                                    ...
                                    <div class="btn-group btn-group-xs" role="group" aria-label="Extra-small button group">
                                        <a href="javascript:void(0);" class="btn btn-default waves-effect waves-float waves-green restore_single_btn" item_id="{{$hotel->id}}">
                                            <i class="zmdi zmdi-edit" title="Restore"></i>
                                        </a>
                                    </div>
                                    <div class="btn-group btn-group-xs" role="group" aria-label="Extra-small button group">
                                        <a href="javascript:void(0);" class="btn btn-default waves-effect waves-float waves-red delete_single_btn" item_id="{{$hotel->id}}">
                ...
                <form id='del_form' method="POST" action="{{url('admin/hotel/destroy')}}" style="display: none">
                    {{ csrf_field() }}
                    {{method_field('DELETE')}}
                </form>
                <form id='res_form' method="POST" action="{{url('admin/hotel/restore')}}" style="display: none">
                    {{ csrf_field() }}
                    {{method_field('PATCH')}}
                </form>
            </div>
...
@parent
    ...
    <script type="text/javascript">
        $('.hotel_list').on('click', '.delete_single_btn', function(event) {
            event.preventDefault();           
            var id= $(this).attr('item_id');
            if(confirm('Are you sure to delete this hotel details permanently.')){   
                var action=$('#del_form').attr('action');
                $('#del_form').attr('action', action+'/'+id);
                $('#del_form').submit();
            }
        });
        $('.hotel_list').on('click', '.force_del_all_btn', function(event) {
            event.preventDefault(); 
            if(confirm('Are you sure to delete all Trash hotels.')){   
                var action=$('#del_form').attr('action');
                $('#del_form').submit();
            }
        });
        $('.hotel_list').on('click', '.restore_all_btn', function(event) {
            event.preventDefault(); 
            if(confirm('Are you sure to restore all Trash Hotels.')){   
               var action=$('#res_form').attr('action');
               $('#res_form').submit();
            }
        });
        $('.hotel_list').on('click', '.restore_single_btn', function(event) {
            event.preventDefault(); 
            var id= $(this).attr('item_id');
            if(confirm('Are you sure to restore this Hotel.')){   
                var action=$('#res_form').attr('action');
