...
@section('admin_container')
    <div class="container-fluid">
        ...
        <div class="row clearfix">
            <div class="col-lg-12">
                <div class="card pagelink_lists">
                    <div class="header">
                       <h4>Links&nbsp;<span class="badge badge-primary">{{$all_count}}</span></h4>
                       <button type="button" class="btn btn-raised btn-primary waves-effect rami_admin_add_new_btn"><a href="{{url('admin/pagelink/'.$page_id.'/create')}}">Add more link</a></button>
                    ...
                    <div class="body table-responsive">
                        <table class="table table-bordered table-striped table-hover dataTable pack_table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{++$loop->index}}</td>
                                    <td><span class="text-muted">{{$link->pagelink_title}}</span></td>   
                                    <td><span class="text-muted">{{$link->pagelink_url}}</span></td>
                                    <td>
                                        <div class="btn-group btn-group-xs" role="group" aria-label="Extra-small button group">
                                            <a href="{{ url('admin/pagelink/'.$link->id.'/edit') }}" class="btn btn-default waves-effect waves-float waves-green">
                                                <i class="zmdi zmdi-edit" title="Edit"></i>
                                            </a>
                                        </div>
                                        <div class="btn-group btn-group-xs" role="group" aria-label="Extra-small button group">
                                            <a href="javascript:void(0);" class="btn btn-default waves-effect waves-float waves-red pagelink_del_btn" item_id="{{$link->id}}">
                ...
                <form id='del_pagelink_form' method="POST" action="{{url('admin/pagelink')}}" style="display: none">
                    {{ csrf_field() }}
                    {{method_field('DELETE')}}
                </form>
            </div>
...
@parent
    ...
    <script type="text/javascript">
        $('.pagelink_lists').on('click', '.pagelink_del_btn', function(event) {
            event.preventDefault();           
            var id= $(this).attr('item_id');
            if(confirm('Are you sure to delete this page.')){   
             var action=$('#del_pagelink_form').attr('action');
             $('#del_pagelink_form').attr('action', action+'/'+id);
             $('#del_pagelink_form').submit();
           }
        });
