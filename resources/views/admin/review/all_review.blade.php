        ...
        <div class="row clearfix">
            <div class="col-lg-12">
                <div class="card review_list">
                    <div class="header">
                       <h4>Reviews&nbsp;<span class="badge badge-primary">{{$all_count}}</span></h4>
                       <button type="button" class="btn btn-raised btn-primary waves-effect rami_admin_add_new_btn"><a href="{{url('admin/review/create')}}">Add-Reviews</a></button>
                    </div>
                    <div class="body table-responsive">
                        <table class="table table-bordered table-striped table-hover dataTable pack_table">
                            ...
                            @foreach($reviews as $review)
                                <tr>
                                    <td>{{++$loop->index}}</td>
                                    @if(!empty($review->hotel))
                                        <td>{{$review->hotel->hotel_name}}</td>
                                    @else
                                        <td> </td>                                    
                                    @endif
                                    <td>{{$review->title_review}}</td>        
                                    <td>{{$review->review_date}}</td>
                                    <td>{{$review->email}}</td>
                                    <td>
                                    <div class="btn-group btn-group-xs" role="group" aria-label="Extra-small button group">
                                        <a href="{{ url('admin/review/'.$review->id.'/edit') }}" class="btn btn-default waves-effect waves-float waves-green"><i class="zmdi zmdi-edit" title="Edit"></i></a>
                                    </div>
                                    <div class="btn-group btn-group-xs" role="group" aria-label="Extra-small button group">
                                        <a href="javascript:void(0);" class="btn btn-default waves-effect waves-float waves-red review_del_btn" item_id="{{$review->id}}"><i class="zmdi zmdi-delete" title="delete"></i></a>
                                    </div>
                                    </td>
                                </tr>
                            @endforeach
                ...
                <form id='del_review_form' method="POST" action="{{url('admin/review')}}" style="display: none">
                    {{ csrf_field() }}
                    {{method_field('DELETE')}}
                </form>
            </div>
...
@parent
    ...
    <script type="text/javascript">
        $('.review_list').on('click', '.review_del_btn', function(event) {
            event.preventDefault();           
            var id= $(this).attr('item_id');
            if(confirm('Are you sure to delete this Review.')){   
             var action=$('#del_review_form').attr('action');
             $('#del_review_form').attr('action', action+'/'+id);
             $('#del_review_form').submit();
           }
        });
