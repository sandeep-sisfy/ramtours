...
@endphp
        ...
        <div class="row clearfix">
            <div class="col-lg-12">
                <div class="card room_price_list">
                    <ul class="nav nav-tabs">
                        @section('nav-tabs')
                            ...
                            <li class="nav-item"><a class="nav-link" data-toggle="tab" href="{{url('admin/room/gallery/'.$room_id)}}" onclick="window.location.href='{{url('admin/room/gallery/'.$room_id)}}'">Gallery</a></li>
                             <li class="nav-item"><a class="nav-link active" data-toggle="tab" onclick="window.location.href='{{url('admin/room/'.$room_id.'/room_stock')}}'">Room Stock</a></li>
                            <li class="nav-item"><a class="nav-link" data-toggle="tab" onclick="window.location.href='{{url('admin/room/'.$room_id.'/room_prices')}}'">Room Price</a></li>
                    ...
                    <div class="header">
                        <h4>Room Stock for "{{ $room_name }}" &nbsp;<span class="badge badge-primary">{{$all_count}}</span></h4>
                            <button type="button" class="btn btn-raised btn-primary waves-effect rami_admin_add_new_btn"><a href="{{url('admin/room/'.$room_id.'/room_stock/create')}}">Add-Room-Stock</a></button>
                    ...
                    <div class="body table-responsive">
                        <table class="table table-hover m-b-0">
                            ...
                            <tbody>                                
                                    @foreach($room_stocks as $stock)
                                    <tr>
                                        <td>{{++$loop->index}}</td>
                                        <td><span class="text-muted">
                                                {{ $stock->stock_start_date }}
                                            </span></td>
                                        <td><span class="text-muted">
                                                {{ $stock->stock_end_date}}</span></td>
                                        <td><span class="text-muted">
                                            {{$stock->room_available   }}</span>
                                        </td>
                                        <td>
                                            <a href="{{ url('admin/room_stock/'.$stock->id.'/edit') }}" class="btn btn-default waves-effect waves-float waves-green">
                                                <i class="zmdi zmdi-edit" title="Edit"></i>
                                            </a>                                        
                                            <a href="javascript:void(0);" class="btn btn-default waves-effect waves-float waves-red room_del_btn" item_id="{{$stock->id}}">
                ...
                <form id='del_room_form' method="POST" action="{{url('admin/room_stock')}}" style="display:none">
                    {{ csrf_field() }}
                    {{method_field('DELETE')}}
                </form>
            </div>
