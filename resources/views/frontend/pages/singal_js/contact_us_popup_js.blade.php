<script type="text/javascript">
    ...
    function submit_contact_form(){
      $.ajax({
        url: "{{url('submit-contact-form')}}",
        type: 'POST',
        data: {_token:'{{csrf_token()}}', first_name_contact: $('[name=first_name_contact]').val(), last_name_contact: $('[name=last_name_contact]').val(),email_contact: $('[name=email_contact]').val(),phone_contact: $('[name=phone_contact]').val(),msg_contact: $('[name=msg_contact]').val()},
          beforeSend:function(){
            $('.rt_loader').show();
          }
      })
      .done(function(res) {
         if(res.status=='success'){
          ...
          $('#rt_warning .rt_errormsg').html('תודה רבה שפנית לרם נסיעות  <br> קיבלנו את פנייתך  <br>נציג מטעמנו ישוב אליכם בהקדם האפשרי בדרכלל עד 24 שעות מרגע קבלת הפנייה');
    ...
    $('.testimonial_div').on('click', '.rami_load_more_btn', function(event) {
      ...
      $.ajax({
        url: '/load_more',
        type: 'POST',
        data: {_token:'{{csrf_token()}}', page:page,type:1,no_of_element:4},
      })
      .done(function(res) {        
        if(res.status=='success'){
