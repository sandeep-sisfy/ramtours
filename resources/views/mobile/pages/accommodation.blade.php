...
<section class="rt-info">
	<div class="container">
		<div class="row">
			<div class="col-sm-12">
				<h3 class="rtpkghead">אפשרויות לינה {{$loc_name}}</h3>
						...
						<div class="card-header" id="headingOne">
							<h5 class="mb-0">
								<button class="btn btn-link" data-toggle="collapse" data-target="#collapseOne"
									aria-expanded="true" aria-controls="collapseOne">סינון על פי קטגוריות: </button>
	...
	<div class="container">
		<div class="row">
			...
			<div class="col-sm-12 rt-inner hotel_div">
				...
				<div class="rt-packframes filterable" data-type="{{ implode(unserialize($hotel->hotel_type),'-')}}"
					...
					<div class="accomm-right col-6 col-sm-6">
						...
						<div class="inst-approv">
							<img src="{{rami_get_file_url($hotel->card->image)}}" alt="{{$hotel->card->card_title}}"
								class="img-fluid">
						</div>
						@endif

						<a href="{{url('/accommodation/'.$hotel['id'])}}"><img
								src="{{rami_get_hotel_single_image($hotel['id'])}}" alt="" class="img-fluid"></a>
			...
			<div class="col-sm-12 text-center load_more_hotels_div">
				...
				<button id="load_more_packs" class="btn btn-primary btn-lg btn-block view-more load_more_hotels"
					type="submit" page_attr="2">ראה עוד</button>
			</div>
			@endif
		</div>
	</div>
...
<script type="text/javascript">
	$('.load_more_hotels_div').on('click', '.load_more_hotels', function (event) {
		...
		$.ajax({
			url: '/hotel_load_more',
			type: 'POST',
			data: { _token: '{{csrf_token()}}', page: page, no_of_element: 9, hotel_location: hotel_location, is_mobile: 1 },
		})
			.done(function (res) {
