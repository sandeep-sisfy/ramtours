<table width="100%">
      <tr>
         <td colspan="2"><br/>
               ...
               <br/>
            מסמך זה אינו מהווה אישור סופי להזמנתך, הזמנתך כרוכה באישור סופי מול ספקי התיירות וחברת התעופה, נציגי רם נסיעות ותיירות יחזרו אליכם תוך 48 שעות לאישורה הסופי
            <br/>
            <hr />
            <br/>
         </td>
        ...
        <thead>
         ...
         <tbody id="order_line_items">
            ...
            @if(!empty($flight_schedule))
               <tr class="item">
                  ...
                  <td class="quantity" width="40%">
                      ...
                      @if(!empty($flight_schedule->up_departure_time))
                        <strong>מאריך המראה - הלוך  :&nbsp;&nbsp;</strong>{{rami_get_require_date_time_format($flight_schedule->up_departure_time,'Y-m-d h:i')}} <br />
                      @endif
                      @if(!empty($flight_schedule->up_arrival_time))
                        <strong>מתאריך נחיתה - הלוך  :&nbsp;&nbsp;</strong>{{rami_get_require_date_time_format($flight_schedule->up_arrival_time, 'Y-m-d h:i')}} <br />
                      @endif
                     @if(!empty($downflight->flight_number))
                      ...
                      @if(!empty($flight_schedule->down_departure_time))
                        <strong>מאריך המראה - הלוך  :&nbsp;&nbsp;</strong>{{rami_get_require_date_time_format($flight_schedule->down_departure_time, 'Y-m-d h:i')}} <br />
                      @endif
                      @if(!empty($flight_schedule->down_arrival_time))
                        <strong>מתאריך נחיתה - הלוך  :&nbsp;&nbsp;</strong>{{rami_get_require_date_time_format($flight_schedule->down_arrival_time, 'Y-m-d h:i')}} <br />
                      @endif
                     </td>
                  </tr>
               @endif
               ...
               <tr class="item">
                      <td width="25%"><h2>שולמו באתר באשראי  </h2></td>
                      <td width="40%">&nbsp;</td>
                      <td width="15%">&nbsp;</td>
                      <td width="15%">
               ...
               <tr class="item">
                      <td width="25%"><h2>שהכמות הכוללת   </h2></td>
                      <td width="40%">&nbsp;</td>
                      <td width="15%">&nbsp;</td>
                      <td width="15%">
               ...
               <tr class="item">
                      <td width="25%"><h2>הסכום שנותר   </h2></td>
                      <td width="40%">&nbsp;</td>
                      <td width="15%">&nbsp;</td>
                      <td width="15%">
