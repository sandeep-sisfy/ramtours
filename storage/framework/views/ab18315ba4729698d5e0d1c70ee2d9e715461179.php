<script type="text/javascript">
  ...
  var cart_id='<?php echo e(strtotime(date('d-m-Y h:i:s')).rand(0,99)); ?>';
  function setup_cart(){
  var package_type='5';
  ...
  // }
  $.ajax({
    url: '<?php echo e(url('cart-setup')); ?>',
    type: 'POST',
    data: {_token: _token, package_type:package_type, cart_id:window.cart_id, adults:rami_pakage_adults, childs:rami_pakage_childs, infants:rami_pakage_infants, flight:'<?php echo e($all_flight['id']); ?>'},
  })
  .done(function(res) {
  ...
  return true;
   ...
   $('.rami_decr_btn').click(function(event) {
    ...
    $('body').on('click', '.checking_cart', function(event) {
      ...
      $.ajax({
        url: '<?php echo e(url('verify-cart')); ?>',
        type: 'post',
        data: {_token: '<?php echo csrf_token(); ?>', cart_id:window.cart_id},
      })
      .done(function(res) {
         if(res.status=='success'){
...
</script><?php /**PATH /home/newssurl/domains/new.s248.upress.link/public_html/resources/views/frontend/pages/singal_js/flight_details_js.blade.php ENDPATH**/ ?>